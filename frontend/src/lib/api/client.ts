...
export async function apiClient<T = unknown>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Use backend's error message if available, otherwise use generic messages
      let userMessage = error.message
      if (error.status === 401) {
        userMessage = error.message || 'Authentication required. Please log in.'
      } else if (error.status === 403) {
        userMessage = error.message || 'You do not have permission to perform this action.'
      } else if (error.status === 404) {
        userMessage = error.message || 'The requested resource was not found.'
      } else if (error.status === 429) {
        // Use backend's specific error message (quota exhausted vs rate limit)
        // Only fall back to generic if backend didn't provide a message
        userMessage = error.message || 'Too many requests. Please try again later.'
      } else if (error.status === 500) {
        userMessage = error.message || 'Server error. Please try again later.'
      } else if (error.status === 503) {
        userMessage = error.message || 'Service temporarily unavailable. Please try again later.'
      }
